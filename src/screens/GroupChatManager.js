...
export default function GroupChatManager(props){
    ...
    async function uploadImage(){
        try{
            if (selectImage) {
                ...
                const { url } = await uploadImagetwo(selectImage, `Images/${groupID}`, "backGround")
                console.log('photo uploaded successfully')
                console.log(url)
                updateDoc(doc(db,"groups",groupID) ,{backGround:url}).then(()=>{
